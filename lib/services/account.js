...
class AccountServiceClient {
	...
	*/
	...
	/**
	 * Calls account::getCurrentAccount
	 * Gets the current account
	 * @returns {Object} Active account
	 * @async
	*/
	async getCurrentAccount() {
		const serviceData = this.services.account.client.endpoints.getCurrentAccount;
		const headers = Object.assign(this.commonHeaders, serviceData.headers);

		headers.Authorization = format(headers.Authorization, { accessToken: this.tokens.access });

		const activeAccount = await got(serviceData.href, { headers }).json();
